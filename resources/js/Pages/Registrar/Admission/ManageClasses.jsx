...
export default function ManageClasses({ sections, flash }) {
    ...
    const handleDelete = () => {
        router.delete(route('registrar.admission.classes.destroy', deleteId), {
            onSuccess: () => {
            ...
            <div className="py-6">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="mb-6 flex justify-between items-center">
                        ...
                        <Link
                            href={route('registrar.admission.classes.create')}
                    ...
                    <div className="bg-white rounded-lg shadow overflow-hidden">
                        <div className="p-4 border-b">
                            <h2 className="text-lg font-semibold mb-4">View Classes Info</h2>
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-50">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Max Students</th>
                                ...
                                {filteredSections.length > 0 ? (
                                    filteredSections.map((section, index) => (
                                        <tr key={section.id} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-6 py-4 text-sm text-gray-900">{section.grade?.name || 'N/A'}</td>
                                            <td className="px-6 py-4 text-sm text-gray-900">{section.name}</td>
                                            <td className="px-6 py-4 text-sm text-gray-900">{section.capacity || 50}</td>
                                            ...
                                            <td className="px-6 py-4 text-sm">
                                                <div className="flex gap-2">
                                                    <Link
                                                        href={route('registrar.admission.classes.edit', section.id)}
                            ...
                            <div className="px-6 py-4 border-t">
                                <div className="flex justify-between items-center">
                                    <div className="text-sm text-gray-600">
                                        Showing {sections.from || 0} to {sections.to || 0} of {sections.total || 0} entries
                                    </div>
                                    <div className="flex gap-2">
                                        {sections.links && sections.links.map((link, index) => (
                                            <Link
                                                key={index}
                                                href={link.url || '#'}
