...
import ParentLayout from '@/Layouts/ParentLayout'; // Using ParentLayout
import {
    AcademicCapIcon,
    CheckBadgeIcon,
    ArrowRightIcon,
...
export default function RegistrationIndex({ auth, student, academicYear, existingRegistration, streams, nextGrade }) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        post(route('parent.registration.store', student.id)); // Using parent route with student ID
    };

    const requiresStream = nextGrade?.level === 11 || (student?.grade?.level === 10 && nextGrade?.level === 11);

    return (
        <ParentLayout auth={auth} header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Annual Registration</h2>}>
            <Head title={`Registration - ${student.user.name}`} />
                        ...
                        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex items-center justify-between">
                            <div>
                                <h1 className="text-2xl font-bold text-gray-900">
                                    Registration for <span className="text-blue-600">{student.user.name}</span>
                                </h1>
                                <p className="text-gray-500 mt-1">
                                    Academic Year: <span className="font-semibold text-gray-700">{academicYear?.name || 'Upcoming'}</span>
                                </p>
                            </div>
                        ...
                        {existingRegistration ? (
                            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center animate-fade-in">
                                ...
                                <p className="text-gray-500 max-w-md mx-auto mb-8 text-lg">
                                    The registration for <strong>{student.user.name}</strong> has been submitted. Status: <span className="font-semibold text-blue-600">{existingRegistration.status || 'Pending'}</span>.
                                </p>
                                <Link
                                ...
                                <div className="lg:col-span-1 space-y-4">
                                    <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                                        ...
                                        <div className="space-y-4">
                                            <div className="p-3 bg-blue-50 rounded-xl border border-blue-100">
                                                <div className="text-xs text-blue-500 uppercase font-bold mb-1">Current Grade</div>
                                    ...
                                    <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-2xl p-6 shadow-sm border border-amber-100">
                                        ...
                                        <p className="text-sm text-amber-800 leading-relaxed">
                                            Please confirm the grade level. If a stream selection is required (Grade 11), please choose carefully.
                                        </p>
                                    </div>
                                </div>
                                        ...
                                        <div className="p-8 space-y-6">
                                            <div>
                                                ...
                                                <p className="text-xs text-blue-600 mt-2 font-medium flex items-center gap-1">
                                                    ...
                                                    {nextGrade ? 'Automatic Promotion Target' : 'Continuing in Current Grade'}
                                                </p>
                                            </div>
