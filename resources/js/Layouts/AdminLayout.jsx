...
export default function AdminLayout({ children }) {
    ...
    const navigation = [
        { name: 'Admin Overview', href: route('admin.dashboard'), icon: Squares2X2Icon },
        { name: 'User Management', href: '#', icon: UsersIcon },
        { name: 'System Settings', href: '#', icon: Cog6ToothIcon },
        { name: 'Reports & Analytics', href: '#', icon: ChartPieIcon },
        { name: 'Audit Logs', href: '#', icon: ShieldCheckIcon },
    ];
                                ...
                                <div className="w-10 h-10 rounded-full bg-white p-0.5 shadow-2xl flex-shrink-0 flex items-center justify-center transform group-hover:scale-110 transition-all duration-300 ring-4 ring-white/10">
                                    <div className="w-full h-full rounded-full bg-[#0F172A] flex items-center justify-center overflow-hidden border border-white/20">
                                        <img
                                            ...
                                            src={auth?.user?.profile_photo_url || `https://ui-avatars.com/api/?name=${auth?.user?.name || 'Admin'}&background=0F172A&color=fff`}
