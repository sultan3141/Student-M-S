                    ...
                    <nav className="px-3 py-2 space-y-1 overflow-y-auto">
                        {navigation.map((item) => (
                            <Link
                                ...
                                href={item.href}
                                onClick={() => setSidebarOpen(false)}
                                className={`flex items-center space-x-3 px-3 py-2.5 rounded-lg text-xs font-bold transition-all ${currentPath === item.href || currentPath.startsWith(item.href)
                                ...
                                <div className="w-10 h-10 rounded-full bg-white p-0.5 shadow-2xl flex-shrink-0 flex items-center justify-center transform group-hover:scale-110 transition-all duration-300 ring-4 ring-white/10">
                                    <div className="w-full h-full rounded-full bg-[#0F172A] flex items-center justify-center overflow-hidden border border-white/20">
                                        <img
                                            ...
                                            src={`https://ui-avatars.com/api/?name=${auth?.user?.name || 'Parent'}&background=0F172A&color=fff`}
                            ...
                            >
                                <Menu.Items className="absolute right-0 top-full mt-0 w-52 origin-top-right divide-y divide-gray-100 rounded-b-[1.5rem] bg-white shadow-2xl ring-1 ring-black ring-opacity-5 focus:outline-none overflow-hidden z-50 transform">
                                    <div className="bg-[#1D4ED8] p-3 flex flex-col items-center">
                                        ...
                                        <h3 className="text-white font-black text-xs tracking-tighter uppercase mb-1">Guardian Portal</h3>
                                        <p className="text-blue-200 text-[8px] font-bold truncate w-full text-center opacity-80 px-2 line-height-none">
                                            {auth?.user?.name || 'Guardian Name'}
                                        </p>
                                    </div>
                                            ...
                                            <Menu.Item>
                                                {({ active }) => (
                                                    ...
                                                    >
                                                        ...
                                                        <span className="text-[7.5px] font-black uppercase tracking-wider">Family</span>
                                                    </Link>
                                                )}
                                            </Menu.Item>
                                            <Menu.Item>
                                                {({ active }) => (
                                                    <button
                                                        ...
                                                        className={`${active ? 'bg-amber-600 scale-[1.02]' : 'bg-amber-500'} flex items-center justify-center space-x-1 p-1.5 rounded-lg text-white transition-all shadow-md h-full w-full`}
                                                    >
                                                        <KeyIcon className="h-3 w-3" />
                                                        <span className="text-[7.5px] font-black uppercase tracking-wider">Security</span>
                                                    </button>
                                                )}
                                            </Menu.Item>
                                        </div>
